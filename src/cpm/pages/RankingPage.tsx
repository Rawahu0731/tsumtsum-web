...
import { conditionKey } from '../utils/condition';
...
export default function RankingPage({ plays }: RankingPageProps) {
  ...
  const rows = useMemo(() => {
    ...
    map.forEach((value, key) => {
      ...
      const itemsKeys = new Set(groupPlays.map((play) => itemsKey(play.items)));
      const items = itemsKeys.size === 1
        ? itemsLabel(groupPlays[0].items)
        : ['Mixed'];
      ...
      list.push({
        ...
        terminal: groupPlays[0].terminal,
        items,
        avg: stats.avg,
        median: median(cpms),
        max: stats.max,
  ...
  return (
    <div className="ranking-root">
      <header className="ranking-header">
        <div>
          <h1>Ranking</h1>
          <p>Compare efficiency by character, skill, terminal, and items.</p>
        </div>
      ...
      <div className="ranking-cards">
        {rows.map((row, index) => (
          <article key={row.key} className="ranking-card">
            ...
            {row.terminal && <div className="card-terminal">{row.terminal}</div>}
